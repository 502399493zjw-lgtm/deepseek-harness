...
flowchart LR
  ...
  pkg_dsh_codex_shared_pool["dsh-codex_shared_pool"]
  svc_openAICodex["ctx.openAICodex<br/>Codex product composition service"]
  ...
  pkg_dsh_codex_shared_pool --> svc_openAICodex
  pkg_e2b --> svc_e2b
  pkg_fs --> svc_fs
  pkg_fs_e2b --> svc_fs
  pkg_fs_local --> svc_fs
...
| `ctx.openAICodex` | `bundle` | `dsh-codex_shared_pool` | - | - | - | 为该 bundle 的模型适配器、浏览器路由、额度子插件和终端命令入口持有唯一一份凭据与偏好状态。 |
