...
{@link DutyVerifier} 收到步骤、模型的一行完成摘要,以及运行时从 run Session 渲染出的有界证据束;检查器从不直接读日志,因此请求就是完整的输入面。基础设施故障会抛出,运行时将其视为一次失败的验证——绝不默默放行。Duty 通过 `verification` 字段选择检查器:`on` 用配置的默认,非空字符串点名某个已注册的 id。
