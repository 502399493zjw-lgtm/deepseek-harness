...
} from '../src/client/CodexQuotaFooter.tsx'
import css from '../src/client/CodexQuotaFooter.module.css'
import { zh, type CodexQuotaLocaleKey } from '../src/client/locales.ts'
...
const SNAPSHOT = {
  currentAccountName: 'codex42@example.com',
...
describe('CodexQuotaFooter', () => {
  it('renders a primary account row with its action and a quieter pool row', async () => {
    const view = render(<CodexQuotaFooter {...props()} />)
    expect(await screen.findByText('codex42@example.com')).toBeTruthy()

    const rows = view.container.querySelectorAll(`.${css.current}, .${css.pool}`)
    expect(rows).toHaveLength(2)
    expect(rows[0]?.textContent).toBe('codex42@example.com · 剩余 73% · 重置时间未知')
  ...
  it('opens the Codex settings section from the compact action', async () => {
    const openSettings = vi.fn()
    render(<CodexQuotaFooter {...props({ openSettings })} />)
    await screen.findByText('codex42@example.com')
  ...
  ])('keeps available pool context when the current account cannot be summarized', async (
