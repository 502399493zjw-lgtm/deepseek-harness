...
import CodexQuotaGateway, {
  Config as CodexQuotaConfigSchema,
  type Config as CodexQuotaConfig,
} from '@deepseek-ai/dsh-host-codex-quota'
...
export type { CodexProfileSummary } from './store.ts'
export {
  DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE,
  DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS,
  DEFAULT_OPENAI_CODEX_SEARCH_MODE,
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.inject(['subprocess'], (quotaCtx) => {
    quotaCtx.plugin(CodexQuotaGateway, config.quota ?? {})
  })
