...
import type { ModelMenuPreferenceOwner } from './FastModeModelPreference.tsx'
import { FastModeTriggerIcon } from './FastModeTriggerIcon.tsx'
import type { ModelTriggerPrefixOwner } from './FastModeTriggerIcon.tsx'
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    /**
     * Register compact content rendered before the model-selector trigger
     * label. Each component receives the active Session model owner, list
     * registrations render together, and no content appears when the list is
     * empty.
     */
    'conversation.input.model.trigger.prefix': {
      kind: 'list'
      scope: 'session'
      owner: ModelTriggerPrefixOwner
    }
    /**
     * Register one preference row in the model-selector panel. Each component
     * receives the active Session model owner, list registrations render
     * together, and the preference region is absent when the list is empty.
     */
    'conversation.input.model.preference': {
      kind: 'list'
      scope: 'session'
      owner: ModelMenuPreferenceOwner
    }
  }
