...
提供方支持多个用户授权的 ChatGPT 账号。账号的存储顺序就是全局额度优先顺序：每次发送 Codex 模型请求前，提供方都会从第一个 profile 开始检查，并使用对应额度未明确耗尽的第一个 profile。因此，调整顺序会在新对话和已有对话的下一次请求生效；正在进行的请求仍使用其当前凭据。
...
打开**设置 → OpenAI Codex**即可添加、查看或移除 DSH 持有的 ChatGPT OAuth profile，并查看每个 profile 的实时 Codex 限额。左栏就是额度优先顺序。选择 profile 卡片只会切换详情；选择**优先使用**才会确认该 profile 为全局优先账号。Host 会以原子方式把它移到首位，且只有刷新读取确认新顺序后，**优先**标记才会移动。当前优先账号仍保留同一个操作，因此重复点击不会产生额外变化。更新失败时会保留旧标记，并继续提供重试操作。

每次发送 Codex 模型请求前，分配都会重新从顺序首位开始。只有所选模型对应的额度桶或账号的个人工作区限额明确为 `0%` 时，提供方才会跳过该 profile；额度元数据缺失或暂时无法读取时，仍会保留高优先级 profile 的使用资格。`gpt-5.3-codex-spark` 使用 `codex_spark` 额度桶，其他 Codex 模型使用 `codex`。如果所有 profile 都已明确耗尽，已有对话会保留当前 profile，尚未绑定的对话会使用第一个 profile。确认调整顺序后，所有对话都会在下一次请求采用新顺序，但不会中断或重放正在进行的请求。

关闭授权窗口只会取消尚未完成的登录，已存储的 profile 保持不变，**添加账号**操作会恢复可用。账号标签依次使用 OAuth profile 的 `name` 和 `email`。DSH 凭据位于 Harness home，与 Codex CLI/Desktop 凭据相互独立。
...
稳定提示词与工具 schema 保留普通的可复用前缀。WebSocket 上下文复用可以通过 `previous_response_id` 只发送严格续接的增量；历史改写、压缩、Fork、连接中断或进程重启会发送完整上下文。当额度分配在两次请求之间切换了对话账号时，插件会关闭该 Session 由提供方持有的续接状态，让下一次请求使用新凭据发送完整上下文。

## 已知限制与延期工作

- **两个凭据存储**：DSH 模型 profile 与官方 Codex home 使用相同的 `name`、`email` 展示规则，但仍是独立凭据存储。重排 DSH 模型 profile 不会重排 `quota.accountHomes`；侧边栏当前账号与模型请求优先账号必须一致时，请在两处配置同一个首位账号。
- **只汇总主要额度窗口**：侧边栏汇总官方主要速率限制窗口。每个 profile 的详细限额保留在设置页；次要窗口和余额不会聚合到侧边栏账号池百分比中。
- **只在请求前分配**：分配使用模型请求开始前可读取的额度元数据。它不会重试已经失败的请求，不会因认证或提供方错误轮换账号，也不会预测非零剩余额度能否完成请求。额度数据缺失时会有意保留高优先级账号的使用资格。
- **对话绑定仅在当前进程保留**：每个对话实际使用的 profile 只在运行中的进程内保留。进程重启后，下一次请求会重新按照持久化的全局 profile 顺序解析。
