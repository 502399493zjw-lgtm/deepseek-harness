...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.input.model.preference',
    kind: 'list',
    scope: 'session',
    summary: 'Register one preference row in the model-selector panel.',
    doc: 'Register one preference row in the model-selector panel. Each component\nreceives the active Session model owner, list registrations render\ntogether, and the preference region is absent when the list is empty.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [
      'codex_shared_pool FastModeModelPreference id \'openai-codex-speed\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.input.model.preference\', () => ctx.slots.register(\n      { name: \'conversation.input.model.preference\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/bundle/dsh-codex_shared_pool/src/client/index.ts:40',
  },
  {
    key: 'conversation.input.model.trigger.prefix',
    kind: 'list',
    scope: 'session',
    summary: 'Register compact content rendered before the model-selector trigger label.',
    doc: 'Register compact content rendered before the model-selector trigger\nlabel. Each component receives the active Session model owner, list\nregistrations render together, and no content appears when the list is\nempty.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'the runtime itself (built in; always present)',
    occupants: [
      'codex_shared_pool FastModeTriggerIcon id \'openai-codex-fast-mode-bolt\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.input.model.trigger.prefix\', () => ctx.slots.register(\n      { name: \'conversation.input.model.trigger.prefix\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/bundle/dsh-codex_shared_pool/src/client/index.ts:30',
  },
  {
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar.settings\' (client-ui-settings-general), so it exists while that entry is mounted',
    occupants: [
      'codex_shared_pool OpenAICodexSettings id \'openai-codex\'',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: ask_user_question, bash, cordis_define, cordis_run, cordis_stop, cordis_undefine, edit, glob, grep, imagegen, read, skill, todo_write, web_fetch, web_search, write',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.chat.node\' (client-ui-tool), so it exists while that entry is mounted',
    occupants: [
      'codex_shared_pool ImagegenToolView key \'imagegen\'',
