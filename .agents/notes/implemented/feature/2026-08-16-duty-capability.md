...
- Deferred to later iterations: an independent verification layer, per-provider token pricing, cron timezones, the Typert Host Remote contract on `DutyService`, the Client Remote aggregate plus `ui-duty` (the repo's message-feedback package set the precedent of shipping the Host side and deferring the client aggregate), and the keyless snapshot through a runnable example that the testing policy wants for the pinned Chinese kickoff.
