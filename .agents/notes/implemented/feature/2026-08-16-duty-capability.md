...
- Shipped after the first commit: the Typert Host Remote contract on `DutyService`, the `duty` session projection, `ui-duty`, the runnable example with its keyless snapshot, the verification seam with the evaluator provider, per-Duty verifier selection, the human appeal flow over failed verdicts, per-provider token pricing, and cron timezones.
- Deferred to later iterations: per-model token pricing (the map prices per provider today).
